...
public class DIP_Java
{
	...
	try{
	    ...
	    Files.copy(in, Paths.get(Folder_choose.selected_folder+"/Image/all_image/"+baseName+".jpg"));
	    new File(Folder_choose.selected_folder+"/Image/DIP_Java/"+baseName).mkdir();
	    
	    for (Entry<String, ArrayList<Integer>> entry : catalog.map.entrySet()) {
	
	
	File input = new File(Folder_choose.selected_folder+"/Image/all_image/"+baseName+".jpg"); 
	BufferedImage image = ImageIO.read(input);  

	BufferedImage scaledImg = Scalr.resize(image, Method.QUALITY, entry.getValue().get(0),entry.getValue().get(1));
    File compressedImageFile = new File(Folder_choose.selected_folder+"/Image/DIP_Java/"+baseName+"/"+baseName+entry.getKey()+".jpg");
