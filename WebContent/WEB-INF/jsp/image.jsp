<%@ page import="review.image.Imagemagick"%>
<%@ page import="review.image.DIP_Java" %>
<%@ page import="review.image.IMagej" %>
<%@ page import="review.image.Catalog_define" %>
...
<%@ page import="review.image.Folder_choose" %>
...
File theDir = new File(Folder_choose.selected_folder+"/Image/Imagemagick/"+Base);
...
if (!theDir.exists())
{
	...
	tim.make_directory(k);
     ...
     try
     {
	 Catalog_define tc = new Catalog_define();
	 tc.catalogs();
...
for (Map.Entry<String,ArrayList<Integer>> entry : entries) {
 ...
 File imgObj_i = new File(Folder_choose.selected_folder+"/Image/Imagemagick/"+Base+"/"+Base+entry.getKey()+".jpg");
long imgLength_i =  imgObj_i.length();



BufferedImage image = ImageIO.read(new File(Folder_choose.selected_folder+"/Image/Imagemagick/"+Base+"/"+Base+entry.getKey()+".jpg"));//give the path of an image
...
File imgObj_d = new File(Folder_choose.selected_folder+"/Image/DIP_Java/"+Base+"/"+Base+entry.getKey()+".jpg");
long imgLength_d =  imgObj_d.length();


BufferedImage image_d = ImageIO.read(new File(Folder_choose.selected_folder+"/Image/DIP_Java/"+Base+"/"+Base+entry.getKey()+".jpg"));//give the path of an image
...
File imgObj_im = new File(Folder_choose.selected_folder+"/Image/imagej/"+Base+"/"+Base+entry.getKey()+".jpg");
long imgLength_im =  imgObj_im.length();


BufferedImage image_im = ImageIO.read(new File(Folder_choose.selected_folder+"/Image/imagej/"+Base+"/"+Base+entry.getKey()+".jpg"));//give the path of an image
